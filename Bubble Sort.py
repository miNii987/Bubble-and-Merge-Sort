...
class Renderer(Window):
    def __init__(self):
        ...
        for e, i in enumerate(self.x):
            color = hex_to_rgb("#FFC6CF")  
            self.bars.append(Rectangle(100+e*100, 100, 80, i*100, color=color*3, batch=self.batch))
        ...
        for i in range(n-1):
            for j in range(0, n-i-1):
                if self.x[j] > self.x[j+1]:
                    ...
                    self.bars = []
                    for e, i in enumerate(self.x):
                        color = hex_to_rgb("#FFC6CF")  # Change this color code as needed
                        self.bars.append(Rectangle(100+e*100, 100, 80, i*100, color=color*3, batch=self.batch))
                    return
